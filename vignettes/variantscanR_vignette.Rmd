...
other animals if the required data and information for this animal is present. 
...
file containing the variants of interest. Secondly, the VCF file to which the variants of interest are filtered against and thirdly, a BED file for the annotation of variants is needed throughout the worflow. These 3 files require a certain format, which is explained in the following sections.
...
The VCF file already has a certain fixed format and doesn't need adjusting. In fact, please **never edit** a VCF in a word processor by hand, because it **will** screw up the format! 
...
name has to match with that of the VCF file. In our example, the name is _sample_9_. 
